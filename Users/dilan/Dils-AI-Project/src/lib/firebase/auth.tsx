...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchOrCreateAppUser = useCallback(async (fbUser: FirebaseUser): Promise<AppUser | null> => {
    ...
    if (userDocSnap.exists()) {
        ...
        // New user from social sign-in, create Firestore doc
         const username = (fbUser.email?.split('@')[0] || `user_${fbUser.uid.substring(0, 5)}`).replace(/[^a-zA-Z0-9_]/g, '');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (fbUser) => {
      setLoading(true);
      if (fbUser) {
        const appUserData = await fetchOrCreateAppUser(fbUser);
        setUser(fbUser);
  ...
  useEffect(() => {
    ...
    if (!user.emailVerified) {
      // Social logins are auto-verified
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (freshUser?.emailVerified) {
        ...
        const appUserData = await fetchOrCreateAppUser(freshUser);
        setUser(freshUser);
        setAppUser(appUserData);
      }
    }, 5000); 
  ...
  }, [user, loading, fetchOrCreateAppUser]);
